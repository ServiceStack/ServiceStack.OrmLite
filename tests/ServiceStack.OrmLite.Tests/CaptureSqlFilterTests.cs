        ...
        public void Can_capture_all_Single_Apis()
        {
            ...
            using (var db = OpenDbConnection())
            {
                ...
                Assert.That(captured.SqlStatements.Last().NormalizeSql(),
                    ...
                    Or.EqualTo("select id, firstname, lastname, age from (select ssormlite1.*, rownum rnum from (\r select id, firstname, lastname, age  from person where (age = 42) order by person.id) ssormlite1\r where rownum <= 0 + 1) ssormlite2 where ssormlite2.rnum > 0")); //Oracle
